...
typedef struct{
    s_tratte* originale[MAXNR];
    s_tratte* date_sorted[MAXNR];
    s_tratte* code_sorted[MAXNR];
    s_tratte* departure_sorted[MAXNR];
    s_tratte* arrival_sorted[MAXNR];
}s_mix_tratte;
...
void leggi_file(char *fileName, int* NR, s_tratte tratte[], s_mix_tratte* mix_tratte); // LEGGE E STAMPA i contenuti iniziali del log
void selezionaDati(s_tratte tratte[], s_mix_tratte* mix_tratte, int NR, comando_e comando); // Funzione che processa i comandi
...
int main(void)
{
    ...
    leggi_file(name_file, &NR, tratte, &mix_tratte);
    ...
    while (cmd != r_fine && error == 0)
    {
        cmd = leggiComando();
        if (cmd != r_error)
            selezionaDati(tratte, &mix_tratte, NR, cmd);
        else
            error = 1;
    }
    return error;
...
void leggi_file(char *fileName, int* NR, s_tratte tratte[], s_mix_tratte* mix_tratte)
{
    FILE* fp_in;
    int i = 0;
...
void selezionaDati(s_tratte tratte[], s_mix_tratte* mix_tratte, int NR, comando_e comando)
{
