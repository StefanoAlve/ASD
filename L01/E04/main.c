...
#define MAXR 1000//lunghezza massima di righe che potrei avere nel file corse.txt
...
//struct che contiene puntatori di puntatori a struct corsa
typedef struct {
    corsa *ordinatoPerCodice[MAXR];
    corsa *ordinatoPerData[MAXR];
    corsa *ordinatoPerPartenza[MAXR];
    corsa *ordinatoPerArrivo[MAXR];
} pOrdinato;
...
int main(void) {
    ...
    comando_e comando;
    pOrdinato ordinamento;
    pOrdinato *pOrdinamento;
    pOrdinamento = &ordinamento;
    char nomeFile[MAXR];
    corsa vettore_corse[MAXR];
    pFine = &fine;
    ...
    for (int i = 0; i < nRighe; ++i) {
        pOrdinamento->ordinatoPerCodice[i] = &vettore_corse[i];
        pOrdinamento->ordinatoPerData[i] = &vettore_corse[i];
        pOrdinamento->ordinatoPerPartenza[i] = &vettore_corse[i];
        pOrdinamento->ordinatoPerArrivo[i] = &vettore_corse[i];
    }
    //ordino per codice , data, partenza ed arrivo in modo da stampare i dati una volta richiesto e non richiamare le funzioni di ricerca ad ogni iterazione
    ordinaCodice(pOrdinamento->ordinatoPerCodice, nRighe);
    ordinaData(pOrdinamento->ordinatoPerData, nRighe);
    ordinaPartenza(pOrdinamento->ordinatoPerPartenza, nRighe);
    ordinaArrivo(pOrdinamento->ordinatoPerArrivo, nRighe);
    while (!fine && nRighe > 0) {
        comando = leggiComando();
        if (comando != r_errore) {
            selezionaDati(vettore_corse, comando, nRighe, pFine, pOrdinamento);
        }
    }
    return 0;
}
...
int leggiFile(char *nomeFile, corsa vettore_corse[]) {//implemento la funzione
    int righe, i = 0;
    FILE *fp,*fpout;//puntatore a file
    fp = fopen(nomeFile, "r");
    ...
    printf("Questa versione stampa i log in un file di output e mantiene un multiordinamento di vettori per codice, data, partenza ed arrivo.\n");
    ...
    while (!(feof(fp)) && i < righe) {//scansiono file fino a eof
        ...
        fprintf(fpout,"\nRiga numero %d:\n", i + 1);
        fprintf(fpout,"Codice tratta: %s\t", vettore_corse[i].codice_tratta);
        fprintf(fpout,"Partenza: %s\t", vettore_corse[i].partenza);
        fprintf(fpout,"Destinazione: %s\t", vettore_corse[i].destinazione);
        fprintf(fpout,"Data: %s\t", vettore_corse[i].data);
        fprintf(fpout,"Ora partenza: %s\t", vettore_corse[i].ora_partenza);
        fprintf(fpout,"Ora arrivo: %s\t", vettore_corse[i].ora_arrivo);
